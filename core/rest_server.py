...
class RestServer():
    ...
    def run(self):
                ...
                for stager in self.shell.stagers:
                    if str(stager.payload_id) == listener_id and not stager.killed:
                        if len(stager.sessions) > 0 and len([z for z in stager.sessions if not z.killed]) > 0:
                            return jsonify(success=False, error="sessions attached.", sessions=[str(s.id) for s in stager.sessions])
                        else:
        ...
        # sessions
        @rest_api.route('/api/sessions', methods=['GET'])
        def get_sessions():
            all_sessions = []
            for stager in self.shell.stagers:
                all_sessions.extend(stager.sessions)
            if not all_sessions:
                return jsonify(success=False, error="No live sessions.")
        ...
        @rest_api.route('/api/sessions/<int:session_id>', methods=['GET'])
        def get_session(session_id):
            ...
            if not all_sessions:
                return jsonify(success=False, error="No live sessions.")
            if session_id > len(all_sessions) or session_id < 0:
                return jsonify(success=False, error="Not a valid session ID.")
            all_sessions.sort(key=lambda s: s.id)
        ...
        @rest_api.route('/api/sessions/killed', methods=['GET'])
        def get_killed_sessions(session_id):
            all_sessions = []
            for stager in self.shell.stagers:
                all_sessions.extend(stager.sessions)
            if not all_sessions:
                return jsonify(success=False, error="No sessions.")
            all_sessions.sort(key=lambda s: s.id)
            ...
            if not cur_sessions:
                return jsonify(success=False, error="No killed sessions.")
